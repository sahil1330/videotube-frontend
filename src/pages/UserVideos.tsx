...
function UserVideos() {
  ...
  useEffect(() => {
    ...
    async function fetchUserVideos() {
      try {
        const response = await axiosInstance.get(`/videos?sortBy=createdAt&sortType=desc&userId=${userDetails?._id}`);
        ...
        <div className='flex flex-1 flex-col gap-4 p-8 pt-0'>
          {
            videos.length > 0 ? (
              videos.map((video) => (
                <Link to={`/watch/${video._id}`} className="flex flex-col" key={video._id}>
